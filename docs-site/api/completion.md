...
`CompletionItem.matchText` is `filterText ?: label`.
...
Apply order: `textEdit` if present, else insert `insertText`/`label` at the caret. `SNIPPET` uses `insertSnippet`. Then `additionalTextEdits`.

Guide: [Completion](/guide/completion).
